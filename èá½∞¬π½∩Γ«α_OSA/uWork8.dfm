object WorkForm8: TWorkForm8
  Left = 302
  Top = 246
  Width = 752
  Height = 534
  HelpContext = 6
  Caption = #1053#1077#1083#1080#1085#1077#1081#1085#1099#1077' '#1091#1088#1072#1074#1085#1077#1085#1080#1103' - '
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object ScrollBox2: TScrollBox
    Left = 0
    Top = 250
    Width = 744
    Height = 250
    HelpContext = 6
    HorzScrollBar.Smooth = True
    HorzScrollBar.Tracking = True
    VertScrollBar.Smooth = True
    VertScrollBar.Tracking = True
    Align = alClient
    Color = clWhite
    ParentColor = False
    TabOrder = 0
    object Image: TImage
      Left = 0
      Top = 0
      Width = 105
      Height = 105
      Transparent = True
    end
  end
  object ScrollBox1: TScrollBox
    Left = 0
    Top = 0
    Width = 744
    Height = 250
    HorzScrollBar.Smooth = True
    HorzScrollBar.Tracking = True
    VertScrollBar.Smooth = True
    VertScrollBar.Tracking = True
    Align = alTop
    Color = clWhite
    ParentColor = False
    TabOrder = 1
    object Panel1: TPanel
      Left = 57
      Top = 0
      Width = 683
      Height = 246
      Align = alRight
      BevelOuter = bvNone
      Color = clWhite
      TabOrder = 0
      object aEdit: TLabeledEdit
        Left = 575
        Top = 9
        Width = 100
        Height = 21
        EditLabel.Width = 131
        EditLabel.Height = 13
        EditLabel.Caption = #1050#1086#1086#1088#1076#1080#1085#1072#1090#1072' '#1040' '#1086#1090#1088#1077#1079#1082#1072' '#1040#1042
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -12
        EditLabel.Font.Name = 'MS Sans Serif'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        LabelPosition = lpLeft
        ParentShowHint = False
        PopupMenu = MainForm.PopupMenu3
        ShowHint = True
        TabOrder = 0
        Text = '-1'
        OnKeyPress = aEditKeyPress
      end
      object bEdit: TLabeledEdit
        Left = 575
        Top = 37
        Width = 100
        Height = 21
        EditLabel.Width = 131
        EditLabel.Height = 13
        EditLabel.Caption = #1050#1086#1086#1088#1076#1080#1085#1072#1090#1072' '#1042' '#1086#1090#1088#1077#1079#1082#1072' '#1040#1042
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -12
        EditLabel.Font.Name = 'MS Sans Serif'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        LabelPosition = lpLeft
        ParentShowHint = False
        PopupMenu = MainForm.PopupMenu3
        ShowHint = True
        TabOrder = 1
        Text = '1'
        OnKeyPress = aEditKeyPress
      end
      object eEdit: TLabeledEdit
        Left = 575
        Top = 65
        Width = 100
        Height = 21
        EditLabel.Width = 135
        EditLabel.Height = 13
        EditLabel.Caption = #1055#1086#1075#1088#1077#1096#1085#1086#1089#1090#1100' '#1074#1099#1095#1080#1089#1083#1077#1085#1080#1081':'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -12
        EditLabel.Font.Name = 'MS Sans Serif'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        LabelPosition = lpLeft
        ParentShowHint = False
        PopupMenu = MainForm.PopupMenu3
        ShowHint = True
        TabOrder = 2
        Text = '0,02'
        OnKeyPress = aEditKeyPress
      end
      object limEdit: TLabeledEdit
        Left = 575
        Top = 93
        Width = 100
        Height = 21
        EditLabel.Width = 113
        EditLabel.Height = 13
        EditLabel.Caption = #1047#1085#1072#1095#1077#1085#1080#1077' i-'#1086#1084' '#1085#1072' '#1096#1072#1075#1077
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -12
        EditLabel.Font.Name = 'MS Sans Serif'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        LabelPosition = lpLeft
        ParentShowHint = False
        PopupMenu = MainForm.PopupMenu3
        ShowHint = True
        TabOrder = 3
        Text = '10000'
        OnKeyPress = limEditKeyPress
      end
      object PXEdit: TLabeledEdit
        Left = 575
        Top = 122
        Width = 100
        Height = 21
        EditLabel.Width = 129
        EditLabel.Height = 13
        EditLabel.Caption = #1053#1072#1095#1072#1083#1100#1085#1086#1077' '#1087#1088#1080#1073#1083#1080#1078#1077#1085#1080#1077':'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -12
        EditLabel.Font.Name = 'MS Sans Serif'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        LabelPosition = lpLeft
        ParentShowHint = False
        PopupMenu = MainForm.PopupMenu3
        ShowHint = True
        TabOrder = 4
        Text = '0'
        OnKeyPress = aEditKeyPress
      end
      object RunB: TBitBtn
        Left = 576
        Top = 213
        Width = 100
        Height = 25
        Caption = #1042#1099#1095#1080#1089#1083#1080#1090#1100
        TabOrder = 5
        OnClick = RunBClick
        Glyph.Data = {
          
          FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
      end
      object RadioGroup1: TRadioGroup
        Left = 226
        Top = 7
        Width = 201
        Height = 105
        Caption = #1042#1099#1073#1077#1088#1080#1090#1077' '#1084#1077#1090#1086#1076' '#1088#1077#1096#1077#1085#1080#1103
        Items.Strings = (
          #1052#1077#1090#1086#1076' '#1076#1077#1083#1077#1085#1080#1103' '#1086#1090#1088#1077#1079#1082#1072' '#1087#1086#1087#1086#1083#1072#1084
          #1052#1077#1090#1086#1076' '#1093#1086#1088#1076
          #1052#1077#1090#1086#1076' '#1082#1072#1089#1072#1090#1077#1083#1100#1085#1099#1093)
        TabOrder = 6
        OnClick = RadioGroup1Click
      end
      object FuncEdit: TLabeledEdit
        Left = 153
        Top = 181
        Width = 522
        Height = 24
        EditLabel.Width = 116
        EditLabel.Height = 13
        EditLabel.Caption = #1047#1072#1076#1072#1081#1090#1077' '#1092#1091#1085#1082#1094#1080#1102' F(x)='
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -12
        EditLabel.Font.Name = 'MS Sans Serif'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        LabelPosition = lpLeft
        ParentFont = False
        ParentShowHint = False
        PopupMenu = MainForm.PopupMenu3
        ShowHint = True
        TabOrder = 7
        Text = '2*e^x+5*x'
      end
      object PFuncEdit: TLabeledEdit
        Left = 153
        Top = 150
        Width = 522
        Height = 24
        EditLabel.Width = 140
        EditLabel.Height = 13
        EditLabel.Caption = #1047#1072#1076#1072#1081#1090#1077' '#1087#1088#1086#1080#1079#1074#1086#1076#1085#1091#1102' F'#39'(x)='
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -12
        EditLabel.Font.Name = 'MS Sans Serif'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        LabelPosition = lpLeft
        ParentFont = False
        ParentShowHint = False
        PopupMenu = MainForm.PopupMenu3
        ShowHint = True
        TabOrder = 8
        Text = '2*x*e^(x-1)+5'
      end
    end
  end
end
